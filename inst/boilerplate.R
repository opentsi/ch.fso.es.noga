...
library(tsdbapi)
keys <- read_dataset_keys("ch.fso.es.noga")
length(keys)
all_vintages <- tsdbapi::read_ts_history(keys)
...
names(all_vintages) <- sub("^ch\\.fso\\.es\\.noga\\.", "", names(all_vintages))
...
# indpau_meta <- read_dataset_ts_metadata("ch.fso.es.noga")


render_metadata()
meta <- read_metadata(".") # CHANGE BOILERPLATE IN DELOREAN
validate_metadata(meta) # TRUE

## Step 6: write process_data and handle_update
...
## Step 8: Final Checks & Automation
devtools::load_all()
